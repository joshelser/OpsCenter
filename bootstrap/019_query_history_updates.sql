...
CREATE TABLE INTERNAL.TASK_QUERY_HISTORY IF NOT EXISTS (task_start timestamp, success boolean, input variant, output variant, task_finish timestamp, task_run_id text, query_id text, table_name text);

-- Update TASK_QUERY_HISTORY that already exist
BEGIN
    -- IF NOT EXISTS and DEFAULT <value> are mutually exclusive in ALTER TABLE .. ADD COLUMN
    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'TASK_FINISH')) THEN
        ALTER TABLE INTERNAL.TASK_QUERY_HISTORY ADD COLUMN TASK_FINISH TIMESTAMP DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'TASK_RUN_ID')) THEN
        ALTER TABLE INTERNAL.TASK_QUERY_HISTORY ADD COLUMN TASK_RUN_ID TEXT DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'QUERY_ID')) THEN
        ALTER TABLE INTERNAL.TASK_QUERY_HISTORY ADD COLUMN QUERY_ID TEXT DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'TABLE_NAME')) THEN
        ALTER TABLE INTERNAL.TASK_QUERY_HISTORY ADD COLUMN TABLE_NAME TEXT DEFAULT NULL;
    END IF;

    IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'RUN')) THEN
        ALTER TABLE INTERNAL.TASK_QUERY_HISTORY RENAME COLUMN RUN TO TASK_START;
    END IF;
END;
