...
ALTER TABLE IF EXISTS INTERNAL.TASK_QUERY_HISTORY ADD COLUMN IF NOT EXISTS TASK_FINISH TIMESTAMP DEFAULT NULL;
ALTER TABLE IF EXISTS INTERNAL.TASK_QUERY_HISTORY ADD COLUMN IF NOT EXISTS TASK_RUN_ID TEXT DEFAULT NULL;
ALTER TABLE IF EXISTS INTERNAL.TASK_QUERY_HISTORY ADD COLUMN IF NOT EXISTS QUERY_ID TEXT DEFAULT NULL;
ALTER TABLE IF EXISTS INTERNAL.TASK_QUERY_HISTORY ADD COLUMN IF NOT EXISTS TABLE_NAME TEXT DEFAULT NULL;
IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_QUERY_HISTORY' AND COLUMN_NAME = 'RUN')) THEN
    ALTER TABLE IF EXISTS INTERNAL.TASK_QUERY_HISTORY RENAME COLUMN RUN TO TASK_START;
END IF;
