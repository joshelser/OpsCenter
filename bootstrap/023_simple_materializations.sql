...
CREATE TABLE INTERNAL.TASK_SIMPLE_DATA_EVENTS IF NOT EXISTS (task_start timestamp, success boolean, table_name varchar, input variant, output variant, task_finish timestamp, task_run_id text, query_id text);
CREATE TABLE INTERNAL.TASK_WAREHOUSE_LOAD_EVENTS IF NOT EXISTS (task_start timestamp, success boolean, warehouse_name varchar, input variant, output variant, task_finish timestamp, task_run_id text, query_id text);

BEGIN
    -- IF NOT EXISTS and DEFAULT <value> are mutually exclusive in ALTER TABLE .. ADD COLUMN

    -- TASK_SIMPLE_DATA_EVENTS
    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_SIMPLE_DATA_EVENTS' AND COLUMN_NAME = 'TASK_FINISH')) THEN
        ALTER TABLE INTERNAL.TASK_SIMPLE_DATA_EVENTS ADD COLUMN TASK_FINISH TIMESTAMP DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_SIMPLE_DATA_EVENTS' AND COLUMN_NAME = 'TASK_RUN_ID')) THEN
        ALTER TABLE INTERNAL.TASK_SIMPLE_DATA_EVENTS ADD COLUMN TASK_RUN_ID TEXT DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_SIMPLE_DATA_EVENTS' AND COLUMN_NAME = 'QUERY_ID')) THEN
        ALTER TABLE INTERNAL.TASK_SIMPLE_DATA_EVENTS ADD COLUMN QUERY_ID TEXT DEFAULT NULL;
    END IF;

    IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_SIMPLE_DATA_EVENTS' AND COLUMN_NAME = 'RUN')) THEN
        ALTER TABLE INTERNAL.TASK_SIMPLE_DATA_EVENTS RENAME COLUMN RUN TO TASK_START;
    END IF;
END;

BEGIN
    -- TASK_WAREHOUSE_LOAD_EVENTS
    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_WAREHOUSE_LOAD_EVENTS' AND COLUMN_NAME = 'TASK_FINISH')) THEN
        ALTER TABLE INTERNAL.TASK_WAREHOUSE_LOAD_EVENTS ADD COLUMN TASK_FINISH TIMESTAMP DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_WAREHOUSE_LOAD_EVENTS' AND COLUMN_NAME = 'TASK_RUN_ID')) THEN
        ALTER TABLE INTERNAL.TASK_WAREHOUSE_LOAD_EVENTS ADD COLUMN TASK_RUN_ID TEXT DEFAULT NULL;
    END IF;

    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_WAREHOUSE_LOAD_EVENTS' AND COLUMN_NAME = 'QUERY_ID')) THEN
        ALTER TABLE INTERNAL.TASK_WAREHOUSE_LOAD_EVENTS ADD COLUMN QUERY_ID TEXT DEFAULT NULL;
    END IF;

    IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'TASK_WAREHOUSE_LOAD_EVENTS' AND COLUMN_NAME = 'RUN')) THEN
        ALTER TABLE INTERNAL.TASK_WAREHOUSE_LOAD_EVENTS RENAME COLUMN RUN TO TASK_START;
    END IF;
END;
