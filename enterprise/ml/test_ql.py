...
sample_data_1 = [
    ...
    {'WAREHOUSE_SIZE': '5X-Large', 'MODEL_RUNTIME_SCORE': 'S', 'COST': 4.5 * 5.724800448},
...
def get_row(data: dict, wh: str) -> pandas.Series:
    d = {"WAREHOUSE_SIZE": wh, "MODEL_RUNTIME_SCORE": data[wh][0], "COST": data[wh][1], 'QUERY_TEXT': "",
         'SCHEMA_NAME': "", 'DATABASE_NAME': ""}
    return pandas.Series(d)


def test_ql(items):
    ql = QlearningIterate(num_states=60, num_actions=3, epsilon=0.01, alpha=0.9, gamma=0.5, reward_func=RewardCandidateB())
    w = 'Large'
    ...
    for _, row in df.iterrows():
        d = {'next':row['next_warehouse_size'], 'current':row['warehouse_size'], 'reward':row['reward'], 'q':ql.learner.q_table[row['state']], 'action':row['action']}
        print(d)
